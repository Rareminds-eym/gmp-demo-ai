-- Step-by-step setup for evaluation_results table
...
CREATE TABLE IF NOT EXISTS evaluation_results (
    ...
    -- Processing metadata
    evaluation_status VARCHAR(20) NOT NULL DEFAULT 'success',
    error_message TEXT,
...
ADD CONSTRAINT unique_email_processed_at UNIQUE (email, processed_at);

-- Step 3: Create indexes one by one
CREATE INDEX IF NOT EXISTS idx_evaluation_results_email 
ON evaluation_results (email);

CREATE INDEX IF NOT EXISTS idx_evaluation_results_processed_at 
ON evaluation_results (processed_at);

CREATE INDEX IF NOT EXISTS idx_evaluation_results_total_score 
ON evaluation_results (total_score);

CREATE INDEX IF NOT EXISTS idx_evaluation_results_case_id 
ON evaluation_results (case_id);

CREATE INDEX IF NOT EXISTS idx_evaluation_results_status 
ON evaluation_results (evaluation_status);
